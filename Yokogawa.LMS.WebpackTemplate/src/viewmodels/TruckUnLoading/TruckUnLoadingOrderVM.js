...
import * as uiServices from '../../services/TruckUnLoading/TruckUnLoadingOrderService';
import uiModels from './TruckUnLoadingOrderModels';
import { CommonGridEditors } from '../../common/CommonGridEditor';
import { StringBuilder } from '../../common/Shared';

function TruckUnloadingOrderList(id, selector) {
    var model = uiModels.getTruckUnLoadingOrderModelList();

    var list = new eworkspace.framework.ListPage(id, model, selector);
    list.setLoadDataHandler(uiServices.getTruckUnloadingOrderList);
...
function TruckUnloadingOrderDetails(id, selector) {
    var model = uiModels.getTruckUnLoadingOrderModel();
    var details = new eworkspace.framework.WorkflowPage(id, model, selector);
    details.setLoadDataHandler(uiServices.getTruckUnloadingOrderDetails);
    ...
    details.onToolBarItemClickById('save', function () {
        ...
        pageSchedule.save(function (data) {
            ...
            if (id === utils.getEmptyGUID()) {
                uiServices.checkTruckUnloadingOrderExist(data.TruckUnloadingOrderNo,
                    function (json) {
                        if (json === true) {
                            uiServices.saveTruckUnloadingOrder(data,
                                function () {
                                    eworkspace.ViewModel.info('Save TruckUnloadingOrder successfully');                                     
                                },
                                function (error) {
                                    eworkspace.ViewModel.info('Save TruckUnloadingOrder error<br/>' + error.responseText);                                      
                                });
                        }

                        if (json === false) {
                            eworkspace.ViewModel.info('TruckUnloadingOrder already exists');
                            return;
                        }
                    });
            } else {
                uiServices.saveTruckUnloadingOrder(data, function () {
                    eworkspace.ViewModel.info('Save TruckUnloadingOrder successfully');                    
                },
                    function (error) {
                        eworkspace.ViewModel.info('Save TruckUnloadingOrder error<br/>' + error.responseText);                         
    ...
    details.onToolBarItemClickById('delete', function () {
        ...
        dialog.show("Delete Record", "Are you sure to delete the record?",
            function () {
                ...
                uiServices.deleteTruckUnloadingOrder(id, function (json) {
                    if (json === "Deleted") {
                        eworkspace.ViewModel.info('Successfully delete the TruckUnloadingOrder');
                        details.parent.navigate();
                    } else {
                        eworkspace.ViewModel.info('TruckUnloadingOrder delete error <br/>' + json);
                        return;
                    }
                });
            }); 
...
function TruckUnloadingOrderForm(id, selector) {
    var template = { Id: 'TruckUnloadingOrderForm', url: "src/view/TruckUnLoading.html" }
    var model = { data: null };    
...
export { TruckUnloadingOrderList, TruckUnloadingOrderDetails,TruckUnloadingOrderForm }
